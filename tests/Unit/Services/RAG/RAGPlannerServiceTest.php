...
class RAGPlannerServiceTest extends UnitTestCase
{
    public function test_decide_parses_json_response(): void
    {
        $service = new RAGPlannerService($this->mockAiResponse(
    ...
    public function test_decide_falls_back_to_aggregate_when_response_is_unstructured(): void
    {
        ...
        $this->assertSame('invoice', $decision['parameters']['model']);
        $this->assertSame('amount', $decision['parameters']['aggregate']['field']);
    }

    public function test_decide_prefers_total_field_and_group_by_status_for_aggregate_questions(): void
    {
        $service = new RAGPlannerService($this->mockAiResponse(
            '{"tool":"db_query","reasoning":"Aggregate invoice totals","parameters":{"model":"invoice"}}'
        ));

        $decision = $service->decide('show total invoices by status', $this->sampleContext([
            'models' => [[
                'name' => 'invoice',
                'description' => 'Invoice model',
                'table' => 'invoices',
                'schema' => ['id' => 'int', 'customer_id' => 'int', 'total' => 'decimal', 'tax' => 'decimal', 'status' => 'string'],
                'tools' => [],
            ]],
        ]), 'gpt-4o-mini');
        ...
        $this->assertSame('total', $decision['parameters']['aggregate']['field']);
        $this->assertSame('status', $decision['parameters']['aggregate']['group_by']);
    }

    public function test_decide_can_request_summary_statistics_when_total_and_average_are_requested(): void
    {
        $service = new RAGPlannerService($this->mockAiResponse(
            'Use aggregate statistics'
        ));

        $decision = $service->decide('show total and average invoice totals', $this->sampleContext([
            'models' => [[
                'name' => 'invoice',
                'description' => 'Invoice model',
                'table' => 'invoices',
                'schema' => ['id' => 'int', 'total' => 'decimal', 'status' => 'string'],
                'tools' => [],
            ]],
        ]), 'gpt-4o-mini');

        $this->assertSame('db_aggregate', $decision['tool']);
        $this->assertSame('summary', $decision['parameters']['aggregate']['operation']);
        $this->assertSame('total', $decision['parameters']['aggregate']['field']);
    }

    public function test_decide_infers_status_and_amount_filters_for_aggregate_questions(): void
    {
        $service = new RAGPlannerService($this->mockAiResponse(
            'Use aggregate statistics'
        ));

        $decision = $service->decide('average paid invoices under 2000', $this->sampleContext([
            'models' => [[
                'name' => 'invoice',
                'description' => 'Invoice model',
                'table' => 'invoices',
        ...
        $this->assertSame('db_aggregate', $decision['tool']);
        $this->assertSame('avg', $decision['parameters']['aggregate']['operation']);
        $this->assertSame('paid', $decision['parameters']['filters']['status']);
        $this->assertSame('total', $decision['parameters']['filters']['amount_field']);
        $this->assertSame(2000.0, $decision['parameters']['filters']['amount_max']);
    }
    ...
    {
        $service = new RAGPlannerService($this->mockAiResponse('not valid json'));
    ...
    {
        config()->set('ai-engine.rag.decision.language_mode', 'ai_first');
