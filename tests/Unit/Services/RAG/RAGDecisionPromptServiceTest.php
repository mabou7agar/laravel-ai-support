...
class RAGDecisionPromptServiceTest extends UnitTestCase
{
    public function test_prompt_builder_includes_business_and_adaptive_sections(): void
    {
        ...
        $feedback = new RAGDecisionFeedbackService($policy);
        $feedback->recordParseFailure('what is the status?', 'tool=db_query');

        $promptService = new RAGDecisionPromptService($policy, $feedback);
    ...
    {
        config()->set('ai-engine.rag.decision.prompt_limits.models', 2);
        config()->set('ai-engine.rag.decision.prompt_limits.nodes', 2);
        config()->set('ai-engine.rag.decision.prompt_limits.model_fields', 1);
