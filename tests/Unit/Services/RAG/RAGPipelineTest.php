...
{
    public function test_pipeline_resolves_collections_retrieves_sources_and_returns_citations(): void
    {
        ...
        Event::assertDispatched(AgentRunStreamed::class, fn (AgentRunStreamed $event): bool => $event->event['name'] === 'rag.started');
        Event::assertDispatched(AgentRunStreamed::class, fn (AgentRunStreamed $event): bool => $event->event['name'] === 'rag.sources_found'
            && ($event->event['payload']['result_count'] ?? null) === 1);
        Event::assertDispatched(AgentRunStreamed::class, fn (AgentRunStreamed $event): bool => $event->event['name'] === 'rag.completed');
    }

    public function test_rag_source_normalizes_vector_graph_hybrid_sql_and_provider_citations(): void
    {
    ...
    public function test_rag_source_normalizes_arrayable_vector_search_results(): void
    {
    ...
    public function test_vector_retriever_reuses_agent_scope_for_rag_filters(): void
    {
    ...
    public function test_response_generator_can_synthesize_answer_when_enabled(): void
    {
...
class TestRAGRetriever implements RAGRetrieverContract
{
    ...
    public function retrieve(array $queries, array $collections, array $options = [], int|string|null $userId = null): array
    {
        return [new RAGSource(
            ...
            title: 'Invoice 10',
            score: 0.99,
            citations: [\LaravelAIEngine\DTOs\RAGCitation::fromArray([
                'type' => 'vector',
                'title' => 'Invoice 10',
                'url' => 'invoice://10',
            ])]
