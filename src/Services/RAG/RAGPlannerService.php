...
class RAGPlannerService
{
    ...
    protected ?RAGAggregateIntentResolver $aggregateIntent = null;
    ...
    public function __construct(
        ...
        ?RAGAggregateIntentResolver $aggregateIntent = null
    )
    {
    ...
    {
        return $this->aggregateIntent()->operation($messageLower, $content);
    }

    protected function inferAggregateField(array $context, ?string $modelName, string $messageLower = ''): string
    {
        return $this->aggregateIntent()->field($context, $modelName, $messageLower);
    }

    protected function inferAggregateGroupBy(array $context, string $messageLower): ?string
    {
        return $this->aggregateIntent()->groupBy($context, $messageLower);
    }

    protected function resolveAggregateGroupField(array $context, string $candidate): ?string
    {
        return $this->aggregateIntent()->resolveGroupField($context, $candidate);
    }

    protected function hasAggregateIntent(string $messageLower, string $content): bool
    {
        return $this->aggregateIntent()->hasIntent($messageLower, $content);
    }

    protected function aggregateIntent(): RAGAggregateIntentResolver
    {
        return $this->aggregateIntent ??= new RAGAggregateIntentResolver();
    }
