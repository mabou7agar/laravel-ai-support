...
/**
 * RAG Analyzer - Uses AI to intelligently understand queries
...
class RAGAnalyzer
{
    ...
    public function analyze(
        ...
        try {
            ...
            Log::channel('ai-engine')->info('RAG analysis', [
                'query' => $query,
            ...
            Log::channel('ai-engine')->error('RAG analysis failed', [
                'query' => $query,
                'error' => $e->getMessage(),
            ]);
...
PROMPT;
    ...
    protected function buildContextSummary(array $conversationHistory): string
    {
        ...
        $messageLimit = config('ai-engine.rag.context_messages', 6);
        $contentLimit = config('ai-engine.rag.context_content_length', 200);
