...
class RAGDecisionPolicy
{
    ...
    {
        return ['sum', 'avg', 'min', 'max', 'count', 'summary'];
    }

    public function decisionPromptTemplatePath(): ?string
    {
        $value = function_exists('config')
            ? config('ai-engine.rag.decision.template_path')
            : null;
    ...
    public function decisionLanguageMode(): string
    {
        ...
        if (function_exists('config')) {
            try {
                $value = config('ai-engine.rag.decision.language_mode', 'hybrid');
            } catch (\Throwable) {
                $value = 'hybrid';
            }
        }
    ...
    public function decisionAdaptiveFeedbackEnabled(): bool
    {
        return (bool) (function_exists('config')
            ? config('ai-engine.rag.decision.adaptive_feedback.enabled', true)
            : true);
    }
    ...
    {
        return (bool) (function_exists('config')
            ? config('ai-engine.rag.decision.adaptive_feedback.persistence.enabled', true)
            : true);
    }
    ...
    public function decisionFeedbackStoreTable(): string
    {
        return (string) (function_exists('config')
            ? config('ai-engine.rag.decision.adaptive_feedback.persistence.table', 'ai_prompt_feedback_events')
    ...
    {
        return (string) (function_exists('config')
            ? config('ai-engine.rag.decision.adaptive_feedback.cache_key', 'ai_engine:rag_decision_feedback')
    ...
    public function decisionFeedbackWindowHours(): int
    {
        $hours = (int) (function_exists('config')
            ? config('ai-engine.rag.decision.adaptive_feedback.window_hours', 48)
            : 48);
    ...
    public function decisionAdaptiveMaxHints(): int
    {
        $count = (int) (function_exists('config')
            ? config('ai-engine.rag.decision.adaptive_feedback.max_hints', 4)
            : 4);
    ...
    public function decisionBusinessContext(): array
    {
        $context = function_exists('config')
            ? config('ai-engine.rag.decision.business_context', [])
            : [];
    ...
    {
        return (string) (function_exists('config')
            ? config('ai-engine.rag.decision.policy_store.default_key', 'decision')
            : 'decision');
    }
    ...
    {
        return (bool) (function_exists('config')
            ? config('ai-engine.rag.decision.policy_store.enabled', true)
            : true);
    }
    ...
    {
        return (bool) (function_exists('config')
            ? config('ai-engine.rag.decision.policy_store.auto_seed_default', true)
            : true);
    }
    ...
    {
        return (string) (function_exists('config')
            ? config('ai-engine.rag.decision.policy_store.table', 'ai_prompt_policy_versions')
    ...
    {
        $hours = (int) (function_exists('config')
            ? config('ai-engine.rag.decision.policy_store.evaluation.window_hours', 168)
            : 168);
    ...
    public function decisionPolicyEvaluationMinSamples(): int
    {
        $minSamples = (int) (function_exists('config')
            ? config('ai-engine.rag.decision.policy_store.evaluation.min_samples', 30)
            : 30);
    ...
    public function decisionPolicyEvaluationMinScoreDelta(): float
    {
        $delta = (float) (function_exists('config')
            ? config('ai-engine.rag.decision.policy_store.evaluation.min_score_delta', 1.0)
            : 1.0);
