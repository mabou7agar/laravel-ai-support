...
/**
 * RAG chat service for retrieval-backed message answering.
 */
class RAGChatService
{
    ...
    public function __construct(
        ...
        $this->scopeOptions = $scopeOptions ?? (app()->bound(AIScopeOptionsService::class)
            ...
            : null);
        $this->config = config('ai-engine.rag', []);
    ...
    public function processMessage(
        ...
        try {
            ...
            if ($isPure && $hasCollections) {
                $aggregateData = $this->getSmartAggregateData($availableCollections, $message, $userId, $options);
            ...
            Log::channel('ai-engine')->error('RAG failed', [
                'session_id' => $sessionId,
                'message' => $message,
                'error' => $e->getMessage(),
            ...
            Log::channel('ai-engine')->error('RAG critical error', [
                'session_id' => $sessionId,
                'message' => $message,
                'error' => $e->getMessage(),
                'trace' => config('app.debug') ? $e->getTraceAsString() : null,
    ...
    public function processMessageStream(
        ...
        try {
            ...
            Log::channel('ai-engine')->error('RAG stream failed', [
                'session_id' => $sessionId,
                'message' => $message,
                'error' => $e->getMessage(),
            ]);
        ...
        foreach ($searchQueries as $searchQuery) {
            foreach ($validCollections as $collection) {
                try {
                    ...
                    $enableDbFallback = config('ai-engine.rag.enable_database_fallback', false);
...
PROMPT;
     ...
     */
    public function getSmartAggregateData(array $collections, string $query, $userId = null, array $options = []): array
    {
        ...
        foreach ($collections as $collection) {
            ...
            try {
                ...
                foreach (['tenant_id', 'workspace_id'] as $scopeKey) {
                    if (array_key_exists($scopeKey, $options) && $options[$scopeKey] !== null && $options[$scopeKey] !== '') {
                        $queryFilters[$scopeKey] = $options[$scopeKey];
                    }
                }
    ...
    protected function fallbackDatabaseSearch(string $collection, string $query, int $maxResults = 5, $userId = null, ?array $messageAnalysis = null): \Illuminate\Support\Collection
    {
        ...
        try {
            ...
            if ($messageAnalysis !== null) {
                // Use pre-analyzed intent (already analyzed by LaravelAgentProcessor/ChatService)
                if (
            ...
            $hardLimit = min($maxResults, config('ai-engine.rag.database_fallback_limit', 20));
