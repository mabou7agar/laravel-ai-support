...
class Neo4jRetrievalService
{
    ...
    public function __construct(
        ...
        protected ?Neo4jRetrievalScoringService $scoringService = null
    )
    {
        ...
        if ($this->scoringService === null && app()->bound(Neo4jRetrievalScoringService::class)) {
            $this->scoringService = app(Neo4jRetrievalScoringService::class);
        }
    }
...
CYPHER,
    ...
    {
        return $this->scoringService()->calculateScore($query, $chunkText, $title, $summary);
    }

    protected function blendScore(?float $vectorScore, float $lexicalScore): float
    {
        return $this->scoringService()->blendScore($vectorScore, $lexicalScore);
    }
    ...
    {
        return $this->scoringService()->expandQueryTerms($query);
    }

    protected function scoringService(): Neo4jRetrievalScoringService
    {
        return $this->scoringService ??= new Neo4jRetrievalScoringService();
    }
