...
class BusinessActionPayloadExtractor
{
    ...
    public function extract(
        ...
        if (!config('ai-agent.business_action_payload_extraction.enabled', true)) {
            return null;
        }
        ...
        try {
            $response = $this->ai->generate(new AIRequest(
                ...
                engine: $options['engine'] ?? config('ai-agent.business_action_payload_extraction.engine', config('ai-engine.default', 'openai')),
                model: $options['model'] ?? config('ai-agent.business_action_payload_extraction.model', config('ai-engine.orchestration_model', config('ai-engine.default_model', 'gpt-4o'))),
                maxTokens: (int) ($options['max_tokens'] ?? config('ai-agent.business_action_payload_extraction.max_tokens', 1400)),
                temperature: (float) ($options['temperature'] ?? config('ai-agent.business_action_payload_extraction.temperature', 0.1)),
                metadata: [
                    'context' => 'business_action_payload_extraction',
                    'action_id' => $actionId,
                ]
            ));
        } catch (\Throwable $exception) {
            Log::channel('ai-engine')->warning('Business action payload extraction failed', [
                'action_id' => $actionId,
    ...
    protected function prompt(
        ...
        return implode("\n", array_filter([
            'BUSINESS_ACTION_PAYLOAD_EXTRACTOR',
            'You extract structured payload patches for a business action from a natural conversation.',
