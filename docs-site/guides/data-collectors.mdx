...
description: Full guide for classic and autonomous data collection, from config to API and node federation.
---
...
- `AutonomousCollectorConfig` + `AutonomousCollectorService`: goal-first autonomous collection with tools.
...
If `AI_ENGINE_STANDARDIZE_API_RESPONSES=true`, responses are wrapped in the standard envelope while preserving legacy fields when `AI_ENGINE_API_RESPONSE_PRESERVE_LEGACY=true`.
...
It also normalizes standard API envelope responses, so it works whether legacy top-level keys are preserved or not.
...
php artisan ai-engine:autonomous-collectors --discover --local-only
php artisan ai-engine:autonomous-collectors --discover --remote-only
...
- Run `php artisan ai-engine:node-ping --all` and `php artisan ai-engine:autonomous-collectors --nodes` to verify visibility.
