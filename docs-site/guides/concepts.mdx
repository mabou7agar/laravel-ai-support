...
There are two application-facing PHP APIs:

- `UnifiedEngineManager` for fluent request building and facade usage
- `AIEngineService` for direct `AIRequest` execution

`EngineProxy` is the fluent object returned by `engine()` and `model()`. Driver instantiation is handled only by `DriverRegistry`.
...
- Use **UnifiedEngineManager** for fluent app code and **AIEngineService** for typed internal service code.
